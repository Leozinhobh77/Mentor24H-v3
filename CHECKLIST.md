...
- [x] **Etapa 25 — Financeiro do Negócio** ✅ (25A + 25B completas — 2026-06-10) — fluxo de caixa + despesas · Controle MEI (faturamento vs limite + DAS) · metas + pró-labore
  ...
  - [x] **25B — MEI + Metas + Pró-labore** ✅ — 2026-06-10 (executor-20260610-003): abas **🏛️ MEI** (faturamento anual por competência vs R$ 81k + faixas 70/90% + regra dos 20% + projeção de ritmo · DAS R$ 76,90/dia 20 nos 12 meses com "Paguei" · lembrete DASN 31/05 · config editável `DB.negocioFin`) e **🎯 Metas** (metas faturamento/lucro `DB.metasNeg` · **Pró-labore** "salário do dono" → saída no caixa **+ ponte PF/PJ**: cria entrada em `DB.transacoes` no Híbrido · **Reserva** `DB.reservaNeg` guardar/resgatar). **7 regras novas do Mentor** (das-vence, mei-limite, mei-projecao, cx-negativo, despesa-subindo, meta-batida, prolabore) com NUC 3 tons. Smoke Playwright ✅ 30/30 (faturamento competência conferido, DAS, pró-labore não duplica, reserva, 7 regras, 360px+escuro). Notas: regra cx-negativo duplica projeção inline (minimal); engrenagem usou ícone `pencil` (`settings` não existe em ICONS)
...
*Atualizado a cada etapa. Última: Etapa 25B (Financeiro — MEI + Metas + Pró-labore + 7 regras do Mentor; ponte PF/PJ) ✅ — 2026-06-10 (executor-20260610-003; smoke Playwright 30/30). **Etapa 25 COMPLETA.** Próxima: Etapa 26 (Fidelidade/Cashback + Ficha técnica/Produção) — nichada, ou pular pra Etapa 27 (Auth+Perfil).*
