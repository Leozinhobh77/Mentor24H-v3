const Contatos=(()=>{
  ...
  function tipoRadios(){return`<div class="ct-canal-chips">`+Object.entries(INT_TIPOS).map(([k,v],i)=>`<span class="ct-canal-chip${i===0?' on':''}" data-canal="${k}" onclick="this.closest('.ct-canal-chips').querySelectorAll('.ct-canal-chip').forEach(x=>x.classList.remove('on'));this.classList.add('on')">${v.e} ${v.l}</span>`).join('')+`</div>`}
  ...
  function renderFicha(c){
    ...
    root.querySelector('[data-falei]').onclick=()=>Modal.open('Como foi o contato?',`<div class="fg"><label>Canal</label>${tipoRadios()}</div><div class="fg" style="margin-top:var(--s-3)"><label>Nota (opcional)</label><input class="field" id="fi-nota" placeholder="Sobre o que conversaram?"></div>`,(b)=>{const tipo=b.querySelector('.ct-canal-chip.on')?.dataset.canal||'whatsapp';const nota=b.querySelector('#fi-nota').value.trim()||INT_TIPOS[tipo].l;c.ultimoContato=offset(0);(c.interacoes=c.interacoes||[]).push({data:offset(0),tipo,nota});Toast.show('Registrado! 👍');renderFicha(c);},'Registrar');
